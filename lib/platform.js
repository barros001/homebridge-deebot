...
export default class {
  ...
  addAreaSwitch(accessory, id, name, type, areaIdsOrCoordinates) {
    ...
    areaService
        ...
        .onSet(async (value) => {
          ...
          this.setAreaSwitchTimer();
        });
  }

  setAreaSwitchTimer() {
    if (this.areaSwitchTimer) {
      clearTimeout(this.areaSwitchTimer);
    }

    this.areaSwitchTimer = setTimeout(() => {
      this.areaSwitchTimer = null;
    }, 1000);
  }

  async waitForAreaSwitchTimer() {
  ...
  async internalCleanUpdate(accessory, value) {
    try {
      ...
      // When the clean switch is turned on, it will check for the state of all
      // area switches and clean the areas specified by these switches, if any is on.
      // It works fine when manually setting switch, but it could cause issues when a scene
      // is created. A scene will command multiple switches at the same time, INCLUDING the
      // clean switch. While it works fine, there's technically a possibility that HomeKit will
      // trigger the clean switch and take a little longer to toggle an area switch, thus
      // commanding the robot to go clean the wrong areas. To prevent that, every time an area
      // is toggled, an artificial delay is added to the clean switch. This delay is reset
      // every time an area is toggled. Also add a minimum 1-second delay as it seems to make
      // turning off the 'charge' switch more responsive.
      await this.waitForAreaSwitchTimer();
